// BLR-4000 + SINDT-485 + AFF500RS 三传感器 + UDP发送
// 编译: g++ -std=c++11 -o triple_sensor_sender triple_sensor_sender.cpp -lpthread
// 运行: ./triple_sensor_sender
//      ./triple_sensor_sender /dev/ttyS0 9600 0x53 0x65 0x31
...
void print_usage(const char* prog_name) {
    std::cout << "Usage: " << prog_name << " [port] [baudrate] [addr_BLR] [addr_SINDT] [addr_WIND]" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "==================================================" << std::endl;
    std::cout << "  三传感器 + UDP发送 + 看门狗 (STA)" << std::endl;
    ...
    std::cout << "  AFF500RS  : 0x" << std::hex << (int)addr_WIND << std::dec << std::endl;
        ...
        if (udp.send(&data, sizeof(data))) {
            if (seq % 5 == 0) {
                std::cout << "[STA] seq=" << seq 
                          ...
                          << " 风速=" << std::setprecision(3) << data.wind_speed << "m/s" << std::endl;
            }
        } else {
